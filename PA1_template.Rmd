...
* Mean total number of steps taken per day are `r mean(totalStepsPerDay$steps)`.
...
```{r, fig.cap=" Histogram of the total number of steps taken each day", fig.height=5, fig.width=7}
...
* Mean is `r mean(totalStepsPerDay$steps)`
...
```{r, fig.cap=" Time Series Plot", fig.height=5, fig.width=6.5}
...
* Maximum number of steps are `r maxSteps` during the interval `r maxStepsInterval` (which represents time around 8:35 in the morning).

#### Imputing missing values

* There are a number of days/intervals where there are missing values (coded as NA). The presence of missing days may introduce bias into some calculations or summaries of the data.

* Calculate and report the total number of missing values in the dataset (i.e. the total number of rows with NAs) which comes out to be `r sum(is.na(activityData))`. We also confirm that these missing values are only in the steps column.
...
```{r, fig.cap=" Histogram of the total number of steps taken each day", fig.height=5, fig.width=6.5}
...
* Mean and Median values are still pretty much the same even after imputing the missing value - Mean = ```r mean(totalStepsByDay$steps)` and Median = `r median(totalStepsByDay$steps)`.
...
```{r, fig.cap="Lattice Plot of Average Steps for Each Interval by Weekdays and Weekends", fig.height=3.5, fig.width=7}
library(ggplot2)
